...
class Xici(ISpider):
    ...
    _siteurl = "https://www.xicidaili.com/nn/"
    _headers = {
    ...
    @classmethod
    def start(cls):
        logger.info("start craw proxy from Xici ...")
    ...
    def _getProxyResultList(cls, html):
        if not html:
            logger.error("an error occurred a try craw proxy list in Xici, because html is None")
            return None
        soup = bs(html, "lxml")
        ...
        for tr in trList:
            ...
            try:
                if GeneralTool.isIPAddress(rowList[0]):
                    ...
                    proxyDict.setdefault("type", GeneralTool.checkPortOrType("type", rowList[8]))
                    ...
                    proxyDict.setdefault("md5", GeneralTool.computeMD5(rowList[0],
                                                                       ...
                                                                       GeneralTool.checkPortOrType("type", rowList[8])))
